...
def createPageModule(BASE_DIR, MAIN_DIR, MODULES):
    ...
    choice = st.selectbox(
        f"Select a {BASE_DIR[:-1].lower()} to execute",
        [None] + list(MODULES.keys()),
        key=MAIN_DIR,
    )
    st.divider()
    ...
    if choice in MODULES:
        ...
        try:
            module = importlib.import_module(
                f"src.apps.pages.{BASE_DIR}.{MAIN_DIR}.{module_name}"
            )
            ...
            st.toast(
                f"Function '{module_name}' could not be found in '{module_name}.py'.",
                icon="🚫",
            )
            st.error(f"An error occurred: {e}", icon="🚫")
        ...
        st.info(
            "Star this project on [GitHub](https://github.com/Code-A2Z/jarvis), if you like it!",
            icon="⭐",
        )
            ...
            pages.append(
                st.Page(
                    lambda path=path.split("/")[
                        -1
                    ], folder=folder, MODULES=MODULES: createPageModule(
                        path, folder, MODULES
                    ),
                    title=name,
