...
def env():
    ...
    given_name = getattr(st.user, "given_name", None) or getattr(st.user, "name", None)

    if email == st.secrets["general"]["ADMIN_EMAIL"] and given_name == st.secrets["general"]["ADMIN_NAME"]:
