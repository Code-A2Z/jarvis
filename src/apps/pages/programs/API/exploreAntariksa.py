...
def SpaceNews(NASA_API_KEY):
    date = st.date_input(
        "What day would you like to know ?", max_value=datetime.date.today()
    )
...
def MarsImage(NASA_API_KEY):
    date = st.date_input(
        ...
        min_value=datetime.date(2012, 8, 16),
        max_value=datetime.date(2024, 1, 21),
    )
    ...
    try:
        for index, photo in enumerate(photos):
            ...
            st.image(
                img_url, caption=f"Camera Name : {camera_name} | Date : {date_of_photo}"
            )
    except:
...
def Asteroids(NASA_API_KEY):
    ...
    with col1:
        start_date = st.date_input(
            "Starting Date", value=datetime.date.today() - datetime.timedelta(days=1)
        )
    with col2:
    ...
    try:
        ...
        st.markdown(
            f"##### `Total asteroids between {start_date} and {end_date} are {totalAstro}`"
        )
        st.divider()
        for dateData in neo:
            for index, body in enumerate(neo[dateData]):
                ...
                estimated_diameter = body["estimated_diameter"]["kilometers"][
                ...
                with col3:
                    if is_potentially_hazardous:
                        st.write(
                            f"`Potentially Hazardous : {is_potentially_hazardous}`"
                        )
                ...
                with col5:
                    st.write(
                        f"Close Approach Date : {close_approach_data['close_approach_date_full']}"
                    )
                with col6:
                    st.write(
                        f"Velocity : {close_approach_data['relative_velocity']['kilometers_per_hour']} km/hr"
                    )

                col7, col8, col9 = st.columns(3)
                with col7:
                    st.write(
                        f"Miss Distance : {close_approach_data['miss_distance']['kilometers']} km"
                    )
                with col8:
...
def SolarBodies():
    try:
        ...
        if data3["collection"]["items"]:
            st.image(
                data3["collection"]["items"][0]["links"][0]["href"], caption=f"{body}"
            )
        ...
        with col1:
            ...
            st.markdown(
                f"**Semi-Major Axis**: {data2.get('semimajorAxis', 'Data not available')} km"
            )
            st.markdown(
                f"**Perihelion**: {data2.get('perihelion', 'Data not available')} km"
            )
            st.markdown(
                f"**Aphelion**: {data2.get('aphelion', 'Data not available')} km"
            )
            st.markdown(
                f"**Eccentricity**: {data2.get('eccentricity', 'Data not available')}"
            )
            st.markdown(
                f"**Inclination**: {data2.get('inclination', 'Data not available')}°"
            )
        ...
        with col2:
            ...
            if mass:
                st.markdown(
                    f"**Mass**: {mass.get('massValue', 'Data not available')} × 10^{mass.get('massExponent', '')} kg"
                )
            else:
            ...
            if vol:
                st.markdown(
                    f"**Volume**: {vol.get('volValue', 'Data not available')} × 10^{vol.get('volExponent', '')} km³"
                )
            else:
                st.markdown("**Volume**: Data not available")

            st.markdown(
                f"**Density**: {data2.get('density', 'Data not available')} g/cm³"
            )
            st.markdown(
                f"**Gravity**: {data2.get('gravity', 'Data not available')} m/s²"
            )
            st.markdown(
                f"**Mean Radius**: {data2.get('meanRadius', 'Data not available')} km"
            )
        ...
        if data2.get("moons"):
            ...
            moons = [
                moon.get("moon", "Data not available") for moon in data2.get("moons")
            ]
            st.markdown(" | ".join(moons))

        with st.expander("🔍 More Details"):
            st.markdown(
                f"**Equatorial Radius**: {data2.get('equaRadius', 'Data not available')} km"
            )
            st.markdown(
                f"**Polar Radius**: {data2.get('polarRadius', 'Data not available')} km"
            )
            st.markdown(
                f"**Escape Velocity**: {data2.get('escape', 'Data not available')} m/s"
            )
            st.markdown(
                f"**Flattening**: {data2.get('flattening', 'Data not available')}"
            )
            st.markdown(
                f"**Dimension**: {data2.get('dimension', 'Data not available')}"
            )
            st.markdown(
                f"**Surface Area**: {data2.get('sideralOrbit', 'Data not available')} km²"
            )
            st.markdown(
                f"**Surface Gravity**: {data2.get('sideralRotation', 'Data not available')} m/s²"
            )

            around_planet = data2.get("aroundPlanet")
            if around_planet:
                st.markdown(
                    f"**Orbits Around**: {around_planet.get('planet', 'Data not available')}"
                )
            else:
                st.markdown("**Orbits Around**: Data not available")

            st.markdown(
                f"**Discovered By**: {data2.get('discoveredBy', 'Data not available')}"
            )
            st.markdown(
                f"**Discovery Date**: {data2.get('discoveryDate', 'Data not available')}"
            )
            st.markdown(
                f"**Alternative Name**: {data2.get('alternativeName', 'Data not available')}"
            )
            st.markdown(
                f"**Average Temperature**: {data2.get('avgTemp', 'Data not available')} K"
            )
...
def exploreAntariksa():
    ...
    choice = st.selectbox(
        ...
        [None, "Space News", "Mars Image", "Asteroids", "Solar Bodies"],
    )
    NASA_API_KEY = (
        os.environ.get("NASA_API_KEY", "") or st.secrets["api_key"]["NASA_API_KEY"]
    )
