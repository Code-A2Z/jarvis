...
def authenticateSpotify():
    client_id = (
        ...
        or st.secrets["spotify"]["SPOTIFY_CLIENT_ID"]
    )
    client_secret = (
        os.environ.get("SPOTIFY_CLIENT_SECRET")
        or st.secrets["spotify"]["SPOTIFY_CLIENT_SECRET"]
    )
...
def music():
    ...
    with col1:
        search_query = st.text_input(
            "Enter artist, album, or track name", placeholder="Type here..."
        )
    with col2:
