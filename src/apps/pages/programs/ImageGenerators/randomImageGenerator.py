...
def getImage(height, width, isGrayscale, blur):
    ...
    try:
        ...
        if response.status_code == 200:
            fileFormat = response.headers.get("Content-Type", "image/jpeg").split("/")[
                -1
            ]
...
def randomImageGenerator():
    choice = st.selectbox(
        ...
        list(resolutions.keys()) + ["Custom Resolutions"],
    )
    if choice == "Custom Resolutions":
        col1, col2 = st.columns(2)
        with col1:
            width = st.number_input(
                "Select image width", min_value=0, max_value=2400, value=400, step=10
            )
        with col2:
            height = st.number_input(
                "Select image height", min_value=0, max_value=2400, value=300, step=10
            )
    elif choice in resolutions:
        resolution = st.selectbox(
            "Select a resolution", list(resolutions[choice].keys())
        )
    ...
    if st.button("Find"):
        image_bytes, encoded_image, fileFormat = getImage(
            height, width, isGrayscale, blur
        )
        if image_bytes:
            st.image(
                image_bytes,
