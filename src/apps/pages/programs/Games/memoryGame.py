...
def memoryGame():
    ...
    if "best_score" not in st.session_state:
        st.session_state.best_score = float(
            "inf"
        )  # Use infinity as the initial best score
    ...
    def handle_card_click(index):
        ...
        if (
            st.session_state.first_selection is not None
            and st.session_state.second_selection is not None
        ):
            # Optionally, you could add a mechanism to delay the checking or visual feedback here
            st.session_state.disable_buttons = (
                False  # Re-enable buttons after processing
            )
    ...
    for i in range(GRID_SIZE):
        for j in range(GRID_SIZE):
            idx = i * GRID_SIZE + j
            if (
                st.session_state.revealed[idx]
                or st.session_state.first_selection == idx
                or st.session_state.second_selection == idx
            ):
                # Show the number if the card is revealed or currently selected
                cols[j].button(
                    str(st.session_state.numbers[idx]), key=idx, disabled=True
                )
            else:
    ...
    with col1:
        st.write(
            f"**Best:** {int(st.session_state.best_score) if st.session_state.best_score != float('inf') else 'N/A'}"
        )
    ...
    if st.session_state.matches_found == NUM_PAIRS:
        st.markdown(
            "<h1 style='text-align: center; font-size: 48px; color: #4CAF50;'>Congratulations, you won!</h1>",
            unsafe_allow_html=True,
        )
