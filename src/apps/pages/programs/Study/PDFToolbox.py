...
def readPDF():
    ...
    if file:
        ...
        numPage = st.number_input(
            "From which page to start reading?",
            format="%d",
            min_value=1,
            max_value=len(reader.pages),
        )
...
def splitPDF():
    ...
    if file:
        ...
        start_page = st.number_input(
            "Start page", min_value=1, max_value=len(pdf_reader.pages), value=1
        )
        end_page = st.number_input(
            "End page",
            min_value=start_page,
            max_value=len(pdf_reader.pages),
            value=len(pdf_reader.pages),
        )
        pdf_writer = PyPDF2.PdfWriter()
...
def encryptPDF():
    password = st.text_input(
        "Enter password", type="password", placeholder="your encryption password"
    )
    ...
    if st.button("Encrypt PDF") and password:
        ...
        with open(output_pdf, "rb") as f:
            st.download_button(
                "Download Encrypted PDF", f, file_name="encrypted_output.pdf"
            )
...
def decryptPDF():
    ...
    if st.button("Decrypt PDF") and password and file:
        ...
        if pdf_reader.decrypt(password):
            ...
            with open(output_pdf, "rb") as f:
                st.download_button(
                    "Download Decrypted PDF", f, file_name="decrypted_output.pdf"
                )
        else:
