...
def showTrendingAssets():
    ...
    asset_type = st.selectbox(
        "Select an asset:", ["Cryptocurrency", "NFTs", "Categories"]
    )
    st.divider()
        ...
        if categories:
            for i, cat in enumerate(categories):
                with st.container():
                    ...
                    cols[2].markdown(
                        format_price_change(
                            cat["data"]["market_cap_change_percentage_24h"]["usd"]
                        )
                    )
...
def cryptoConversion(from_coin, to_coin):
    response = requests.get(
        f"{BASE_URL}/simple/price?ids={from_coin}&vs_currencies={to_coin}"
    )
    if response.status_code == 200:
        data = response.json()
        price = data[from_coin][to_coin]
        st.success(
            f"**{from_coin.upper()} = {price:,.2f} {to_coin.upper()}**", icon="✅"
        )
    else:
        st.error("API call not successful. Please try again later.", icon="🚨")
...
def showTopCryptocurrency():
    response = requests.get(
        f"{BASE_URL}/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10"
    )
    if response.status_code == 200:
        data = response.json()
        st.divider()
        for coin in data:
            ...
            with col2:
                ...
                st.write(
                    f"**24h Price Change**: {format_price_change(coin['price_change_percentage_24h'])}"
                )
                st.divider()
    else:
        st.error("API call not successful. Please try again later.", icon="🚨")
...
def showCryptoMarketOverview():
    ...
    if response.status_code == 200:
        ...
        st.info(
            f"Market Capitalization Change (24h) {format_price_change(data['data']['market_cap_change_percentage_24h_usd'])}",
            icon="ℹ️",
        )
        ...
        top_10_coin_volume_pairs = sorted(
            coin_volume_pairs, key=lambda x: x[1], reverse=True
        )[:10]
        ...
        top_10_volume_percentages = [
            vol / sum(top_10_volumes) * 100 for vol in top_10_volumes
        ]
        ...
        st.plotly_chart(
            px.pie(
                ...
                values=[
                    data["data"]["market_cap_percentage"][coin]
                    for coin in data["data"]["market_cap_percentage"]
                ],
...
def showCompanyHoldings():
    ...
    if response.status_code == 200:
        ...
        col3.metric(
            label="Market Cap Dominance", value=f"{data['market_cap_dominance']:.2f}%"
        )
...
def cryptoCurrency():
    ...
    if option == "Search Cryptocurrency":
        query = st.text_input(
            f"Enter the coin name or symbol", placeholder="e.g. Bitcoin, BTC"
        )
