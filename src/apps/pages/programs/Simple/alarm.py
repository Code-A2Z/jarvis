...
def alarm():
    ...
    with col4:
        message_option = st.radio(
            ...
            ("Predefined Message", "Custom Message"),
            key="msg_opt",
        )
        if message_option == "Predefined Message":
            ...
            alarm_message = st.selectbox(
                "Choose a predefined message", predefined_messages, key="predef_msg"
            )
        else:
            alarm_message = st.text_area(
                "Enter your custom message", "Time's up!", key="custom_msg"
            )

    with col5:
        note_option = st.radio(
            "Choose Note/Link Option", ("None", "Custom Note/Link"), key="note_opt"
        )
        if note_option == "Custom Note/Link":
            alarm_note = st.text_area(
                "Enter your link or note here", "", key="custom_note"
            )
        else:
    ...
    if st.button("Set Alarm"):
        ...
        if alarm_dt <= now:
            st.toast(
                f"(> {datetime.datetime.now().strftime('%H:%M:%S')}) Alarm time must be in the future.",
                icon="⚠️",
            )
