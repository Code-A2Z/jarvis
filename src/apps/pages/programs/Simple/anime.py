    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for anime_details in data["data"][:10]:
                ...
                score = (
                    str(anime_details["score"]) + "/10"
                    if anime_details["score"]
                    else "--"
                )
                synopsis = (
                    anime_details["synopsis"] if anime_details["synopsis"] else "--"
                )
                season = anime_details["season"] if anime_details["season"] else "--"
                year = anime_details["year"] if anime_details["year"] else "--"
                anime_genres = "Genre: " + ", ".join(
                    [genre_name["name"] for genre_name in anime_details["genres"]]
                )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for character_name, character_details in character_map.items():
                ...
                about = (
                    character_details["about"] if character_details["about"] else "--"
                )
                nicknames = ", ".join(
                    [name for name in character_details["nicknames"]]
                    + [character_details["name_kanji"]]
                    if character_details["name_kanji"]
                    else []
                )
...
def anime():
    option = st.selectbox(
        "Select an option", options=["Animes by Genres", "Top Animes", "Top Characters"]
    )
    ...
    if option == "Top Animes":
        ...
        with col3:
            selected_genres = st.multiselect(
                "Select genres", options=[genre for genre in ALL_GENRES]
            )
        if selected_genres:
