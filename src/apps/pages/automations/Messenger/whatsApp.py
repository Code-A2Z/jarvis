...
def whatsApp():
    ...
    bottom = st.number_input(
        "Enter the bottom range of phone numbers:", min_value=0, value=0
    )
    top = st.number_input(
        "Enter the top range of phone numbers:", min_value=bottom + 1, value=bottom + 1
    )
    ...
    if uploaded_file is not None:
        if st.button("Send WhatsApp Message"):
            if message == "":
                st.warning(
                    'Please upload a CSV file with a column named "Phone Number".',
                    icon="⚠️",
                )
                return
            ...
            if top > len(df):
                st.warning(
                    "The top range of phone numbers exceeds the total number of phone numbers in the CSV file.",
                    icon="⚠️",
                )
                return
            ...
            if "phone_number" not in df.columns:
                st.warning(
                    'Please upload a CSV file with a column named "phone_number".',
                    icon="⚠️",
                )
                return
