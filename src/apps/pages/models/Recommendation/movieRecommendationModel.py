...
def movieRecommendationModel():
    movie = st.selectbox(
        "Select a movie from dropdown", [None] + [m for m in movies_list]
    )
    ...
    if st.button("Show Recommend") and movie is not None:
        recommended_movies, recommended_movies_data = recommend(
            movie, num_movies_recommend
        )
        if recommended_movies:
            for i, (m, p) in enumerate(
                zip(recommended_movies, recommended_movies_data)
            ):
                st.divider()
                ...
                with col2:
                    ...
                    st.write(
                        f"**Spoken Languages**: {', '.join([l['name'] for l in p['spoken_languages']])}"
                    )
                    st.write(
                        f"**Genres**: {', '.join([g['name'] for g in p['genres']])}"
                    )
        else:
