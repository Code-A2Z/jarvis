...
youtube_key = config["api_key"]["YOUTUBE_API_KEY"]


def API_Exist():
    if "YOUTUBE_API_KEY" in youtube_key and youtube_key["api_key"]["YOUTUBE_API_KEY"]:
        return True
...
def youtubePlaylist():
    ...
    if API_Exist():
        API_KEY = (youtube_key['api_key']["YOUTUBE_API_KEY"]
                   or os.environ["YOUTUBE_API_KEY"])
        ...
        showInstructions()
        st.info(
            f"You can also go through my [blog post]({BLOG_URL}) for a detailed guide on how to get the YouTube API key.", icon="📝")
        st.error(
            "YouTube API key not found. Please add your API key to the secrets manager.", icon="🚨")
